...
class SMPLifyLoss(nn.Module):
	...
	def forward(self, body_model_output, camera, gt_joints, joints_conf,
		...
		densepose_loss = torch.tensor(0.0)
		#n_faces = len(self.face_idx_to_atlas_i_arr)
		#min_dist = -torch.ones(n_faces, dtype = projected_joints.dtype, device = projected_joints.device)
		#H, W = self.densepose_i_arr.shape
		#min_dist = -torch.ones(H * W, dtype = projected_joints.dtype, device = projected_joints.device)
		min_dist = None
		#utils.gpu_usage('before densepose_loss')
		if self.use_densepose:
			densepose_loss, min_dist = self.densepose_loss_min_dist(projected_joints.shape[0], body_model_output, 
														body_model_faces, camera, min_dist)
			densepose_loss *= self.densepose_loss_weight
			#densepose_loss = torch.tensor(0.0)
		#utils.gpu_usage('after densepose_loss')
	...
	def densepose_loss_min_dist(self, batch_size, body_model_output, body_model_faces, camera, min_dist):
		densepose_loss = 0.0
		...
		for part_id in range(1, 7):
			...
			target_pixel_ids = (target_pixels[:, 0] * W + target_pixels[:, 1]).long()
			...
			dist_matrix = torch.pow(predicted_pixels_tiled - torch.transpose(target_pixels_tiled, 0, 1), 2) # M*N*2
			ones = torch.ones_like(dist_matrix)
			# superset loss
			dist_matrix_min_super, _ = torch.min(torch.sum(torch.relu(dist_matrix - ones), 2), 1)
			#min_dist[face_ids_per_part_masked] = dist_matrix_min.detach()
			# subset loss
			dist_matrix_min_sub, _ = torch.min(torch.sum(torch.relu(dist_matrix - ones), 2), 0)
			#min_dist[target_pixel_ids] = dist_matrix_min.detach()
			print('part id:', part_id, torch.mean(dist_matrix_min_super).data.cpu().numpy(), \
						dist_matrix_min_super.max().data.cpu().numpy(), \
						torch.mean(dist_matrix_min_sub).data.cpu().numpy(), \
						dist_matrix_min_sub.max().data.cpu().numpy())
			densepose_loss += (torch.mean(dist_matrix_min_sub) +  torch.mean(dist_matrix_min_super)) * 5.
			del dist_matrix, predicted_pixels_tiled, target_pixels_tiled
		del face_mask, faces, projected_face_pts, face_pts
