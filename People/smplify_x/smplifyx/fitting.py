...
class SMPLifyLoss(nn.Module):
    ...
    def forward(self, body_model_output, camera, gt_joints, joints_conf,
        ...
        min_dist = None
        if self.use_densepose:
            densepose_loss, min_dist = self.densepose_loss_min_dist(
                projected_joints.shape[0], body_model_output, body_model_faces, camera)
    ...
    def densepose_loss_min_dist(
        ...
        projected_face_pixel_x, projected_face_pixel_y = torch.split(
            projected_face_pts, 1, dim=2)
        ...
        for part_id in range(1, 7):
            ...
            face_mask_per_part = torch.nonzero(
                face_mask[face_ids_per_part])[:, 0]
            ...
            predicted_pixels = torch.stack(
                [predicted_pixel_y, predicted_pixel_x], dim=1)

            N = target_pixels.shape[0]
            M = predicted_pixels.shape[0]
            #print('M, N:', M, N)
            predicted_pixels_tiled = torch.unsqueeze(
                predicted_pixels, dim=1).repeat(
                1, N, 1)  # M*N*2
            #print('predicted_pixels_tiled:', predicted_pixels_tiled.shape)
            target_pixels_tiled = torch.unsqueeze(
                target_pixels, dim=1).repeat(
                1, M, 1)  # N*M*2
            #print('target_pixels_tiled:', target_pixels_tiled.shape)
            dist_matrix = predicted_pixels_tiled - \
                torch.transpose(target_pixels_tiled, 0, 1)  # M*N*2
            #print('dist_matrix:', dist_matrix.shape)
            ones = torch.ones_like(dist_matrix)
            dist_matrix_min, _ = torch.min(
                torch.sum(torch.relu(dist_matrix - ones), 2), 1)
