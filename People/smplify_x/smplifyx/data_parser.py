...
from utils import smpl_to_openpose, group_atlas_i
...
def read_densepose(densepose_fn, smplx_atlas_fn):
    ...
    _, h, w = iuv_arr.shape
    i_arr = group_atlas_i(iuv_arr[0, :, :].flatten())
    i_arr = i_arr.reshape((h, w))
    print('i_arr:', i_arr.shape, np.unique(i_arr))
    ...
    for index in range(len(face_idx_to_atlas_i_arr)):
        i = face_idx_to_atlas_i_arr[index]
        if i != -1:
...
class OpenPose(Dataset):
    ...
    def __init__(self, data_folder, img_folder='images',
        ...
        self.densep_folder = osp.join(data_folder, densep_folder)
        self.smplx_atlas_fn = smplx_atlas_fn
    ...
    def read_item(self, img_path):
        ...
        if self.use_densepose:
            output_dict['densepose_i'] = denspose_i
            output_dict['face_idx_to_atlas_i'] = face_idx_to_atlas_i
